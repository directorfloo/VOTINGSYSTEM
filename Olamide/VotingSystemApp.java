...
public class VotingSystemApp {
  ...
  public  static LocalDate date = LocalDate.now();
   ...
   public static ArrayList<Integer> candidateVotes = new ArrayList<>();
    ...
    static ArrayList<String> highestWinner = new ArrayList<>(); 
    static ArrayList<Integer> highestCandidate = new ArrayList<>();
    ...
    public static void registerCandidates() {
        ...
        for (int counter = 0; counter < candidates.size(); counter++) {
            System.out.println("- " + candidates.get(counter)  
                + " (Age: " + candidateAges.get(counter)  
                + ", Party: " + candidatePartys.get(counter) + ")");
        }
        System.out.println();
    }
        ...
        for (int index = 0; index < voters.size(); index++) {
            ...
            while (true) {
                ...
                if (choice >= 1 && choice <= candidates.size()) {
                    int currentVotes = candidateVotes.get(choice - 1);
                    candidateVotes.set(choice - 1, currentVotes + 1);
    ...
    public static void showResults() {
        ...
        for (int count = 0; count < candidates.size(); count++) {
            System.out.println(candidates.get(count) + " (" + candidatePartys.get(count) + "): " +
                    candidateVotes.get(count) + " votes");

            if (candidateVotes.get(count) > highestVotes) {
                highestVotes = candidateVotes.get(count);
                winner = candidates.get(count);
                highestCandidate.clear();
                highestWinner.clear();
                highestCandidate.add(highestVotes);
                highestWinner.add(winner);
            }
        }
